...
相比很多只提供“未完成 / 已完成”二元切换的待办工具，PlanTick 更强调任务过程管理。每个任务都可以在 `未开始`、`进行中`、`已完成`、`阻塞`、`取消` 之间流转，适合更真实地反映执行状态、识别卡点并管理推进节奏。
...
按清单方式统一管理全部待办，结合分类、截止日期和五种任务状态，适合集中整理尚未完成的事项。

<img src="./doc/images/02.png" width="100%" alt="PlanTick 待办箱视图" />

### 看板

将任务按状态映射到看板列中，便于围绕 `未开始`、`进行中`、`已完成`、`阻塞`、`取消` 五种状态观察整体推进情况。
...
- 多状态任务管理：支持 `未开始`、`进行中`、`已完成`、`阻塞`、`取消` 五种状态，不把任务粗暴压缩成“做完 / 没做完”两类，更适合真实跟踪执行过程。
