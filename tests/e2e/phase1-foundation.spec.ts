...
test('phase 3 我的一天支持按日期切换查看并让计数跟随当前查看日', async ({ page, baseURL }) => {
...
test('phase 4 双设备同步：同一事件的多条 outbox 记录会在推送前折叠为最新版本', async ({ browser, baseURL }) => {
  test.setTimeout(60_000)

  const passphrase = `phase4-duplicate-event-outbox-${Date.now()}-pw`
  const originalTitle = '过时同步标题'
  const updatedTitle = '最新同步标题'
  const viewport = { width: 1440, height: 960 }
  ...
  try {
    ...
    await pageA.evaluate(async ({ originalTitle, updatedTitle }) => {
      const eventId = crypto.randomUUID()
      const earlier = new Date(Date.now() - 60_000).toISOString()
      const later = new Date().toISOString()
      const today = later.slice(0, 10)
      ...
      const workspaceMeta = await new Promise<{ workspaceId: string }>((resolve, reject) => {
        ...
        const request = tx.objectStore('workspace_meta').get('current')
        request.onerror = () => reject(request.error)
        request.onsuccess = () => resolve(request.result)
      })

      const transaction = db.transaction(['events', 'outbox'], 'readwrite')
    ...
    await pageA.reload()
    await openCalendar(pageA)
    await expect(pageA.locator('.calendar-grid').getByText(updatedTitle)).toBeVisible()
    ...
    await expect.poll(async () => pageA.evaluate(async () => {
      ...
      const records = await new Promise<Array<{ entity: string }>>((resolve, reject) => {
      ...
      db.close()
      return records.filter((record) => record.entity === 'events').length
    })).toBe(0)
    ...
    await openCalendar(pageB)
    await getSyncActionButton(pageB).click()
    await expect(pageB.locator('.calendar-grid').getByText(updatedTitle)).toBeVisible()
    await expect(pageB.locator('.calendar-grid').getByText(originalTitle)).toHaveCount(0)
